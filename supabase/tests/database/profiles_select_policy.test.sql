...
SELECT
  is(
    (
      SELECT user_role
      FROM profiles
      WHERE id = tests.get_supabase_uid('william.edward.douglas@blue-moonlight-stream.com')
    ),
    null,
...
SELECT is(
  (
    SELECT user_role
    FROM profiles
    WHERE id = tests.get_supabase_uid('christopher.david.thompson@blue-moonlight-stream.com')
  ),
  'company_teammate',
