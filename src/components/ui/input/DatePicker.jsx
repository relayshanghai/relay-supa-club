  ...
  <div>
    <Controller
      ...
      render={({ field: { value, onChange } }) => (
        <div>
          <label htmlFor={fieldName} className="text-xs text-gray-600 pl-2">{label}</label>
          <input value={value} onChange={val => onChange(val) }type="date" name={fieldName}  className="input-field mb-2" />
        </div>
      )}
    />
