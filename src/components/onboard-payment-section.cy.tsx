...
describe('<OnboardPaymentSection />', () => {
    ...
    it('Handles payment request', () => {
        // note that we can't test the real payment success because stripe will need a 'clientSecret' sent from the backend, so in this test we can just make sure that tne backend is called with the right data and if it returns an error, we can show it to the user
        worker.use(
