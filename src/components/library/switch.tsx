...
export const Switch = ({ beforeLabel, afterLabel, wrapperClassName, className, ...props }: SwitchProps) => {
    return (
        <div className={`flex items-center ${wrapperClassName}`}>
            ...
            <input
                type="checkbox"
                className={`relative h-7 w-[3.25rem] shrink-0 cursor-pointer appearance-none rounded-full border-2 border-transparent bg-gray-100 text-primary-100 outline-1 outline-red-500 ring-1 ring-gray-200 ring-offset-white transition-colors duration-200 ease-in-out before:inline-block before:h-6 before:w-6 before:translate-x-0 before:transform before:rounded-full before:bg-white before:shadow before:ring-0 before:transition before:duration-200 before:ease-in-out checked:bg-primary-50 checked:ring-primary-100 checked:before:translate-x-full checked:before:bg-primary-500 checked:before:shadow-primary-300 checked:hover:bg-primary-100 checked:focus:bg-primary-100${className}`}
                {...props}
            />
