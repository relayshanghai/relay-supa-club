...
export default function SequenceInfluencerTableEmail({
    ...
    influencer,
    sequenceId,
}: Readonly<SequenceInfluencerTableEmailProps>) {
    const { t } = useTranslation();
    const [value, setValue] = useState(influencer.email || '');
    const { setSequenceInfluencerByIndex } = useSequenceInfluencerStore();
    const { updateEmail } = useSequenceInfluencerEmail(sequenceId, influencer.id);
    ...
    return <TableInlineInput onSubmit={onSubmit} value={value} textPromptForMissingValue={t('sequences.addEmail')} />;
}
