import { pgTable, foreignKey, pgEnum, uuid, timestamp, text, bigint, boolean, numeric, jsonb, smallint, doublePrecision, integer, varchar, unique, index, json } from "drizzle-orm/pg-core"
...
export const campaign_creators = pgTable("campaign_creators", {
	...
	added_by_id: uuid("added_by_id").notNull().references(() => profiles.id),
	influencer_social_profiles_id: uuid("influencer_social_profiles_id").references(() => influencer_social_profiles.id, { onDelete: "set null" } ),
...
export const campaign_notes = pgTable("campaign_notes", {
	...
	id: uuid("id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	sequence_influencer_id: uuid("sequence_influencer_id").references(() => sequence_influencers.id, { onDelete: "cascade" } ),
...
export const influencer_categories = pgTable("influencer_categories", {
	...
	influencer_id: uuid("influencer_id").notNull().references(() => influencers.id),
});
...
export const sequence_steps = pgTable("sequence_steps", {
	...
	sequence_id: uuid("sequence_id").notNull().references(() => sequences.id),
...
export const search_parameters = pgTable("search_parameters", {
	id: uuid("id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
...
(table) => {
	return {
		idx_createdat_runat_status_queue: index("idx_createdat_runat_status_queue").on(table.queue, table.run_at, table.status, table.created_at),
...
export const sequence_emails = pgTable("sequence_emails", {
	...
	sequence_influencer_id: uuid("sequence_influencer_id").notNull().references(() => sequence_influencers.id, { onDelete: "cascade" } ),
	sequence_step_id: uuid("sequence_step_id").notNull().references(() => sequence_steps.id),
